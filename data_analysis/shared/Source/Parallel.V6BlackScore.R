...
source(paste0(source.dir, "fetch_and_prepare_data_oldSNPs.R"))
load(prepared_rda)
...
# 1) Map label_split_all indices back to original rows of RA_Newest_with_83SNPs
idx_lab_all_pos <- which(idx_lab_all)# positions of labeled_all_clean in the full frame
idx_unlab_all_pos <- which(idx_unlab_all)

orig_tr_idx     <- idx_lab_all_pos[train_idx_all]    # original row ids for TRAIN (all-race)
orig_te_idx     <- idx_lab_all_pos[test_idx_all]     # original row ids for TEST  (all-race)
...
# original-row indices for Black subsets
blk_tr_idx <- orig_tr_idx[is_black_tr]
blk_te_idx <- orig_te_idx[is_black_te]
blk_ul_idx <- idx_unlab_all_pos[is_black_unlab]

# 3) Sanitize SNP names and check they exist in the big frame
Black_specific_snps_safe <- make.names(Black_specific_snps)
stopifnot(all(Black_specific_snps_safe %in% names(RA_Newest_with_83SNPs)))

# 4) Extract the Z matrices for those SAME Black rows with the "other 10" SNPs
# 0NAs
Z_black_tr_other <- as.matrix(RA_Newest_with_83SNPs[blk_tr_idx, Black_specific_snps_safe, drop = FALSE])

# drop missingness
Z_black_te_other <- as.matrix(RA_Newest_with_83SNPs[blk_te_idx, Black_specific_snps_safe, drop = FALSE])
idx_complete_label_te<-complete.cases(Z_black_te_other)
Z_black_te_other<-Z_black_te_other[idx_complete_label_te, , drop=FALSE]

label_split_black$test$Z<-label_split_black$test$Z[idx_complete_label_te,,drop=FALSE]

# 18 NAs drop missingness
Z_black_unlabel_other <- as.matrix(RA_Newest_with_83SNPs[blk_ul_idx, Black_specific_snps_safe, drop = FALSE])
idx_complete_unlabel <- complete.cases(Z_black_unlabel_other)
Z_black_unlabel_other <- Z_black_unlabel_other[idx_complete_unlabel, , drop=FALSE]
Z_unlabeled_mat_black<-Z_unlabeled_mat_black[idx_complete_unlabel,,drop=FALSE]

storage.mode(Z_black_tr_other) <- "numeric"
storage.mode(Z_black_te_other) <- "numeric"
storage.mode(Z_black_unlabel_other)<-"numeric"

# (optional) keep pretty names
colnames(Z_black_tr_other) <- Black_specific_snps
colnames(Z_black_te_other) <- Black_specific_snps
colnames(Z_black_unlabel_other)<-Black_specific_snps

# quick sanity: same number of rows as your existing black splits
stopifnot(nrow(Z_black_tr_other) == nrow(label_split_black$train$Z))
stopifnot(nrow(Z_black_te_other) == nrow(label_split_black$test$Z))
