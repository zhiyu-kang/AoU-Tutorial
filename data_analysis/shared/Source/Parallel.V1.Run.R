...
label_split_all<-split_data(labeled_all,Z_labeled_mat,seed=2025)
...
#===============================================================================
# 3) SSL: batched parallel perturbations + W-hat + betaSSL
#===============================================================================
...
if (file.exists(ssl.ckpt)) {
  load(ssl.ckpt)   # expects: ibatch, betak_all, Skb_all, Sk_all, h1_all, batch.cut, nbatch, Nperturb
  resume <- TRUE
} else {
  # ---------- prep: Sk_all & h1_all ----------
  T_ssl_prep <- tictoc({
    beta.std_all <- betadelta_all / sqrt(sum(betadelta_all^2))
    lp_all       <- drop(Z_unlabeled_mat %*% beta.std_all)
    sdlp_all     <- sd(lp_all)
    h1_all       <- sdlp_all / (sum(unlabeled.all$DELTA))^0.3
...
if (exists("cl")) {
  parallel::clusterExport(
    ...
    c("init.beta.perturb","Sk_sym_perturb","Sk_sym","W_hat_adaPCA_ridge",
      "label_split_all","KC_all","betadelta_all","Z_unlabeled_mat",
      "unlabeled.all","h1_all","p","nL_tr","n_all","link","dlink"),
    envir = environment()
  )
}
...
T_ssl_loop <- tictoc({
  ...
  for (ibatch in idx) {
    ...
    res.list <- foreach(
      ...
      # standardize & score on unlabeled
      bk.std <- bk / sqrt(sum(bk^2))
      lp_p   <- Z_unlabeled_mat %*% bk.std
    ...
    if (any(!ok)) {
      ...
      for (j in bad_j) {
        ...
        stopifnot(is.numeric(bk), length(bk) == p, all(is.finite(bk)))
        
        s <- sqrt(sum(bk^2)); stopifnot(is.finite(s), s > 0)
        bk.std <- bk / s
        lp_p   <- (Z_unlabeled_mat %*% bk.std)[, , drop = FALSE]
    ...
    # checkpoint
    save(ibatch, betak_all, Skb_all, Sk_all, h1_all, batch.cut, nbatch, Nperturb, file = ssl.ckpt)
    message(sprintf("[SSL] Batch %d/%d done. (perturb %d..%d)",
